 ...
 * @since 1.0
 */
public class CreateUserCompletionDAO {
    ...
    public boolean select(int tempId, UserDTO dto){
        ...
        try{
            ...
            sql = "select user_name,password,email,postal_code,address,cell_number,card_number,"
                    + "card_holder,expiration_date,security_code,user_kana from temp WHERE temp_id = ?";
     ...
     */
    public int insert(String userName,String password,String email,String postalCode,String address,String cellNumber,String nameKana){
        try{
            Connection con = DBConnector2.getConnection("openconnect");
            sql ="INSERT INTO user(user_name,password,email,postal_code,address,cell_number,user_kana,registration_date) VALUES(?,?,?,?,?,?,?,now())";
    ...
    /**
     * openconnectデータベースのuserテーブルから、メールアドレスを条件にユーザーIDを取得するメソッド
     * @param email メールアドレス
     * @return isResult データベースに値を格納することに成功したらresにtrue、失敗したらfalseを返します。
     */
    public boolean selectId(String email){
        boolean isResult = false;
        try{
            Connection con = DBConnector2.getConnection("openconnect");
